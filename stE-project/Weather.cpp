...
// ћожет стоит добавить другую формулу дл€ shift_koeff, как например smoothstep в glsl
Vector3f Light_cycle::get_light_color(double time) {
	...
	// то ли € тупой, то ли проще не написать этот кусок куска ????
	...
	return light_color[0].second; // Ќа вс€кий случай
}

void Light_cycle::load_from_file(ifstream& file) {
	int n;
	file >> n; //  оличество пар
...
void Weather_system::update_leaves() {
	...
	// «десь обновление координат существующих листиков
	...
	// “ут спавн новых листиков (по одному за раз и не чаще чем раз в (cooldown/количество листьев, которые необходимо заспавнить) секунд)
...
// ѕрочитать из файла все состо€ни€ погоды
void Weather_system::load_from_file(ifstream& file) {
	...
	srand(time(0));
	weather_order.push_back(pair<Weather_state*, float>(&states[rand() % counter], 0.0)); // Ќадо добавить погоду в самое начало дл€ корректной работы

	file >> counter; // —читываем листики
	for (int i = 0; i < counter; i++) {
		Sprite_object leaf;
...
// ƒобавить в очередь новые погодные состо€ни€ в количестве num_of_states штук
void Weather_system::create_order(int num_of_states) {
	...
	for (int i = 0; i < num_of_states; i++) {
		...
		float time = last_state_time + itrtr->avg_time * (float(rand() % 100) / 100.f + 0.5f); // ¬рем€ предыдущей погоды + от 0.5 до 1.5 среднего времени следующего
...
void Weather_system::update() {
	...
	// ѕогоду можно обновл€ть каждую игрову секунду, это не должно быть шибко заметноы
	if (last_update_second != game_time->total_seconds) {
		wind_angle += double((rand() % 2) * 2 - 1) * 0.00575958653; //  огда проходит одна секунда, то обновл€ем направление ветра
		...
		// “ут просто ужас, который где то должен быть прописан, смешиваем две погоды
