...
void Game_time::update() {
	if (!running) // ≈сли часы остановлены то ничего не делаем
		return;

	if (running != last_tick_running) { // ≈сл часы не работали, а теперь их запустили то сбрасываем таймер (за врем€ пока игровые часы не тикали там успевает накопитс€ врем€)
		clock.restart();
		return;
	}
